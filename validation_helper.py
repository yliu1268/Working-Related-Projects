...
class BackTester(object):
    ...
    def process(self):
        ...
        max_gain = -1
        index = 0
        output_file = output_folder + '/val_result.txt'
        output_file_seq = output_folder + '/val_seq.txt'
        with open(validation_file, 'r') as f, open(output_file, 'w') as writter, open(output_file_seq, 'w') as writter_seq:
            for line in f:
                
                line_list = line.strip('\n').split('$$$')
                strategy_string = line_list[1]
                
                final_funds = line_list[0].split('\t')[1]
                
                strategy_list = strategy_string.split('  ')
                
                strategy_dict = {}
                for item in strategy_list:
                    strategy_name = item.split(',')[0]
                    strategy_val = item.split(',')[1].strip()
                    strategy_dict[strategy_name] = str(strategy_val)
                ...
                writter_seq.write(final_funds + '\tFF\t' + final_val + '\tBG\t' + '\t'.join(["{:0.2f}".format(elem) for elem in funds_curve_list]) + '\n')
                #print(strategy_dict)
            index += 1
...
if __name__ == '__main__':
    ...
    validation_file = str(sys.argv[1])
    output_folder = str(sys.argv[2])
    data_mode = str(sys.argv[3])
    target_year = int(sys.argv[4])
    pk_file = str(sys.argv[5])
    day_search_file = str(sys.argv[6])
